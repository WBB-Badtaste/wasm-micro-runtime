...
/* Kqueue events */
static struct kevent monitor_events[MAX_EVENTS];
static int monitor_events_num = 0;
static struct kevent change_events[MAX_EVENTS];
                         ...
                         const char *name, attr_container_t *args)
{
    ...
    ev.events = EPOLLIN;
    ev.data.ptr = conn;
    EV_SET(&ev, fd, EVFILT_VNODE, EV_ADD, NOTE_WRITE, 0, NULL)
...
static void* polling_thread_routine (void *arg)
{
    while (polling_thread_run) {
        ...
        n = kevent(kqueuefd, change_events, MAX_EVENTS, monitor_events, monitor_events_num, NULL);
        ...
        for (i = 0; i < n; i++) {
            sys_connection_t *conn
                             = (sys_connection_t *)change_events[i].uintptr_t;
